...
describe('the panel reads every problem with the house it was found in', () => {
  test('the query joins the parent listing by its foreign key column', async () => {
    const { client, selects } = recordingClient();

    await fetchProblems(client);

    expect(selects.join(' ')).toContain('parent:parent_id(name)');
  });

  // Without this the panel cannot tell a room from a part of a combined
  // listing, and would prefix a listing with its neighbour's name.
