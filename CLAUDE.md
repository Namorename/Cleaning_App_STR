...
- **Push в `main` — это прод-деплой панели.** Vercel сам выкатывает в прод
  ...
  Пример: чтение фото сообщений (слой 5 чата, `5a04752`) лежит в `main` с
  19.09, окна 2 (`20260918180000…200000`) в облаке нет, и прод-панель на
  каждой ветке чата получает `PGRST200` — чат не открывается. То же для
  ...
  же. Сейчас это значит: никакого `eas update` до выката окна 2.
