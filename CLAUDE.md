...
- **`db push`, запущенный агентом, не спрашивает подтверждения.** При
  ...
  `--dry-run`, и проверка состава каталога должна быть неотделима от самого
  push, одной командой:
  `[ "$(ls supabase/migrations/*.sql | tail -1)" = "<нужный файл>" ] && npx supabase db push --linked`.
  То же касается выката одной миграции из середины пачки: лишние файлы
  выносятся из каталога, а после возврата сверяются `sha256sum -c`.
